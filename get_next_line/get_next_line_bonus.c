...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/29 18:22:46 by adian             #+#    #+#             */
...
char	*get_next_line(int fd)
{
	...
	if (!buff)
		return (NULL);
	while (byte_read > 0 && !ft_find_n(remains[fd]))
	{
		byte_read = read(fd, buff, BUFFER_SIZE);
		if (byte_read < 0)
			return (free_buff(buff));
		buff[byte_read] = '\0';
		remains[fd] = ft_strjoin(remains[fd], buff);
	}
	free_buff(buff);
	ret_line = ft_return(remains[fd]);
	remains[fd] = ft_save(remains[fd]);
	return (ret_line);
}
